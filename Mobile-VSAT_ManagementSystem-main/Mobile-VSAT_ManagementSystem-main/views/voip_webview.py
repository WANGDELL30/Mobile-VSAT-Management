...
class VoipView(QWidget):
    def __init__(self):
        super().__init__()

        voip_url = os.getenv("VOIP_URL") or os.getenv("voip") or ""
        ...
        url = _to_qurl(voip_url)

        if not url.isValid():
            self.webview.setHtml(
                "<h2>VoIP page not configured</h2>"
                "<p>Set <b>VOIP_URL</b> in <code>.env</code>.</p>"
                "<p>You can use a local file, e.g. <code>assets/voip.html</code></p>"
            )
        else:
